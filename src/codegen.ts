...
const getDecl = (headerFile: string, className: string) => {
  const header = readFileSync(headerFile).toString();
  const classDef: string = header.substring(
    header.indexOf(`${className} : public`),
    header.lastIndexOf(`}`)
  );

  const decl: string = classDef.substring(
    classDef.indexOf(`{`),
    classDef.lastIndexOf(`}`)
  );

  const path = headerFile.split(`\\`);

  path.splice(0, 1);

  const headerName = path.join(`/`);

  return splitDeclMembers(
    decl
      // remove class stuff
      .replace(new RegExp(`public:`, `g`), ``)
      .replace(new RegExp(`private:`, `g`), ``)
      // remove extra brackets
      .substring(decl.indexOf(`{`) + 1, decl.lastIndexOf(`}`) - 1),
    headerName,
    className
  );
};

const splitDeclMembers = (
  decl: string,
  headerFile: string,
  className: string
) => {
  const lines: string[] = decl.split(`\n`);
  const classMembers: ClassMember[] = [];

  lines.forEach((line: string) => {
    if (line.length > 1 && !line.includes(`unk`)) {
      const [member, offsetWithComments]: string[] = line.split(` // `);

      if (!offsetWithComments) return;

      const offset: string = offsetWithComments.split(` `)[0];

      if (!offset.startsWith(`0x`)) return;

      const [type, name]: string[] = member
        .trim()
...
const files: string[] = glob("codegen/gd.h/**/*.h");

for (const file of files) {
  const path = file.split(`\\`);
  const classDef = path[path.length - 1].split(`.h`);

  const decl = getDecl(file, classDef[0]);

  if (decl.members[0]) {
    const codegenOutput: string = `
    //  .｡*ﾟ+.*.｡   Class ${decl.name} [${decl.file}]
    // ✧･ﾟ: *✧･ﾟ:*  Generated by Kyanite SDK. Made with 💖 by Syndicated  *:･ﾟ✧*:･ﾟ✧

    import JSMeow from "jsmeow"
    import { CCNode, CCLayer, CCSprite, CCPoint } from "../../src/classes/cocos2d"

    import { dereference, getPointer, dereferenceStd } from "../../src/utils"

    class ${decl.name} {
        ${decl.members.map((member) => {
        return `
            public get ${member.name}(): ${
            member.type === "bool"
            ? "boolean"
            : member.type === "int" || member.type === "double" || member.type === "float"
            ? "number"
                ...
                member.type !== "std::string"
                    ...
                    },
                    {
                        returnValue: JSMeow.memory.readMemory(
                            getPointer(this) + ${member.offset}, // ${decl.name}->${
            member.name
        };
                            "${
                            member.type === "bool"
    ...
    writeFileSync(
        `codegen/output/${decl.name}.ts`,
        codegenOutput
            .replace(new RegExp(`            ,`, `g`), ``)
            .replace(new RegExp(`cocos2d::`, `g`), ``)
      );
  }
}

// console.log(JSON.stringify(getDecl(input, `PlayLayer`), null, 2));
